...
# 2. 认证
## 2.1 登陆校验流程
![[imgs/Pasted image 20230203131344.png]]
...
![[imgs/Pasted image 20230203133158.png]]
...
![[imgs/Pasted image 20230203184002.png]]

### 2.2.2 认证流程详解
![[imgs/Pasted image 20230203184124.png]]
...
**登录思路**：
①自定义登录接口
	调用 ProviderManager 的方法进行认证如果认证通过生成 jwt 
	把用户信息存入redis中
...
思路图
![[imgs/Pasted image 20230203200141.png]]
...
因为 UserDetailsService 方法的返回值是 UserDetails 类型，所以需要定义一个类，实现该接口，把用户信息封装在其中，当我们调用 UserDetails 方法时，就会调用我们自己的类
```java
@Data  
@AllArgsConstructor  
@NoArgsConstructor  
...
![[imgs/Pasted image 20230204133127.png]]
...
//        通过authenticationManager进行用户认证，里面需要传入UsernamePasswordAuthenticationToken对象，参数为username 和 password        Authentication authenticate = authenticationManager.authenticate(authenticationToken);  
//        认证没通过给出对应提示  
        if (Objects.isNull(authenticate)) {  
            throw new RuntimeException("登录失败");  
        }  
//        认证通过 使用userid生成一个jwt，jwt 存入ResponseResult返回  
...
登录接口放行配置，注入 authenticationManagerBean。
```java
...
![[imgs/Pasted image 20230203133158 1.png]]

这里 token 为空放行的原因是 token 为空代表没有登陆过，所以放行让他去访问数据库进行账号密码的效验。而不是放行他访问资源页面。看上图过滤器位置理解即可。
...
//        这里我们使用三个参数的构造函数，因为三个参数的构造函数把我们的认证信息设置为true  
...
但是要使用它我们需要先开启相关配置。在 Security 配置类上方添加注解。
```java
...
![[imgs/Pasted image 20230204192959.png]]
